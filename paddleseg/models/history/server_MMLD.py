...
class Fusion(nn.Layer):
    def __init__(self, inp1, inp2, reduction=32):
        ...
        self.fc1 = nn.Sequential(
            nn.Conv2D(768 + 64 + 64, 768, 3, padding=1),
            nn.ReLU(),
            nn.Conv2D(768, 768, 1),
            nn.Sigmoid()
        )
        self.fc2 = nn.Sequential(
            nn.Conv2D(768 + 64 + 64, 64, 3, padding=1),
            nn.ReLU(),
            nn.Conv2D(64, 64, 1),
            nn.Sigmoid()
        )
    ...
    def forward(self, x12, x21, x22):  # hill,opt,dem,sar
        ...
        identity_x1 = x12
        identity_x2 = x21  # dem 64
        b1, c1, _, _ = x12.shape
        b2, c2, _, _ = x21.shape
        # 通道注意力
        x1_x2 = paddle.concat([x12, x21, x22], axis=1)
        # x1_x2.shape: [2, 768 + 64 + 64, 256, 256]  [2, 896, 256, 256]
        gap = self.gap(x1_x2)
        # gap.shape: [2, 832, 1, 1]
...
class server_MMLD(nn.Layer):
    ...
    def forward(self, opt, dem, sar):  # 山体阴影图 光学影像 DEM Sar
