...
class Fusion(nn.Layer):
    def __init__(self, inp1, inp2, reduction=32):
        ...
        self.conv_h = nn.Conv2D(64, 1, kernel_size=(7, 1), stride=1, padding=(3, 0))

        self.conv_w = nn.Conv2D(64, 1, kernel_size=(1, 7), stride=1, padding=(0, 3))
    ...
    def forward(self, x2):  # hill,opt,dem,sar
        ...
        avg_h = self.pool_h(x2_gap)  # avg_h.shape: [2, 832, 256, 1]
        avg_w = self.pool_w(x2_gap)  # avg_w.shape: [2, 832, 1, 256]
...
class MMLD_model_class(nn.Layer):
    ...
    def forward(self, hill, opt, dem, sar):  # 山体阴影图 光学影像 DEM Sar
        ...
        # hill_shape = paddle.shape(hill[0])  # Tensor [2, 768, 256, 256]

        sar, sar_pred = self.dem(sar)  # dem.shape: [2, 64, 1024, 1024]
        sar_pred.stop_gradient = True
