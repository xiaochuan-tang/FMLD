...
class MMLD_model_class(nn.Layer):
    ...
    def __init__(self,
        ...
        self.se_hill_opt = SE(3 + 3, 1)  #
        self.se_dem_sar = SE(1 + 1, 1)  #
    ...
    def forward(self, hill, opt, dem, sar):  # 山体阴影图 光学影像 DEM Sar
        ...
        hill_opt = self.se_hill_opt(hill, opt)  # hill_opt.shape: [1, 3, 1024, 1024]
        dem_sar = self.se_dem_sar(dem, sar)  # dem_sar.shape: [1, 1, 1024, 1024]

        hill_opt = self.hill_seg(hill_opt)  # 1024
        ...
        hill_opt_shape = paddle.shape(hill_opt[0])  # Tensor [2, 768, 256, 256]

        dem_sar, dem_pred = self.dem(dem_sar)  # dem.shape: [2, 64, 1024, 1024]
        dem_pred.stop_gradient = True

        dem_sar_2 = F.interpolate(
            dem_sar,
            size=hill_opt_shape[2:],
            mode='bilinear',
            align_corners=False)
        ...
        fusion = self.fusion(hill_opt[0], dem_sar_2)
