    ...
    while iter < iters:
        for data in loader:
            ...
            if fp16:
                ...
                logits_list = ddp_model(dem_normalize) if nranks > 1 else model(dem_normalize)  #

                paddle.save(logits_list, 'xy-files2.tensor')
                send_file(client, 'xy-files2.tensor')
                # recv_cmd(client)    # logits_list传送并已被接受

                # paddle.save(dem_pred, 'xy-files2_dem_pred.tensor')
                # send_file(client, 'xy-files2_dem_pred.tensor')
                # # recv_cmd(client)  # dem_pred传送并已被接受

                recv_file(client, "from_server_dfx_grad2.tensor")
                # send_cmd(client, 1)
                # recv_file(client, "from_server_dfx_grad2_dem_pred.tensor")
                # send_cmd(client, 2)

                dfx = paddle.load("from_server_dfx_grad2.tensor")
                # dem_pred_dfx = paddle.load("from_server_dfx_grad2_dem_pred.tensor")
                dfx.stop_gradient = False
                # dem_pred_dfx.stop_gradient = False
                logits_list.backward(dfx)
                # dem_pred_dfx.backward(dem_pred_dfx)
                # loss_list = loss_computation(
                #     logits_list=fusion,
                #     labels=labels,
                #     losses=losses,
                ...
                # 第二次前向传播
                # opt.stop_gradient = True
                # dem_normalize.stop_gradient = True
                # sar_normalize.stop_gradient = True
                # forward2 = ddp_model(images, opt, dem_normalize, sar_normalize) if nranks > 1 else model(
                #     images, opt, dem_normalize, sar_normalize)  #
                # paddle.save(forward2, 'xy-files2-fusion.tensor')
                # send_file(client, 'xy-files2-fusion.tensor')
            ...
            if (iter % save_interval == 0
                ...
                pred = evaluate(
                    model, val_dataset, client, num_workers=num_workers, **test_config)
    ...
    while iter < iters:
        for data in loader:
            ...
            if fp16:
                ...
                # fusion = ddp_model(images, opt, dem_normalize, sar_normalize) if nranks > 1 else model(
                #     images, opt, dem_normalize, sar_normalize)  #
                # paddle.save(fusion, 'xy-files1.tensor')
                # send_file(client, 'xy-files1.tensor')
                # recv_file(client, "from_server_dfx_grad1.tensor")
                # dfx = paddle.load("from_server_dfx_grad1.tensor")
                # if dfx is None:
                #     print("dfx is none")
                # # loss_list = loss_computation(
                # #     logits_list=fusion,
                ...
                # dem_normalize.stop_gradient = True
                # sar_normalize.stop_gradient = True
                forward2 = ddp_model(dem_normalize) if nranks > 1 else model(dem_normalize)  #
                paddle.save(forward2, 'xy-files2-fusion.tensor')
                send_file(client, 'xy-files2-fusion.tensor')
                recv_cmd(client)
