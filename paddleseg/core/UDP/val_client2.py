...
from paddleseg.core import infer_client2
...
def evaluate(model,
    ...
    with paddle.no_grad():
        for iter, (im, opt, dem, sar, label) in enumerate(loader):
            ...
            if aug_eval:
                pred = infer_client2.aug_inference(
                    model,
                    im,
                    ori_shape=ori_shape,
                ...
                pred = infer_client2.inference(
                    ...
                    stride=stride,
                    crop_size=crop_size)
                paddle.save(pred, "./model_para/xy_files2_test.tensor")
                # print(pred.shape)
                # send_file(client, "xy_files2_test.tensor")
                u.send_string(sock, "begin", far_ip)
                u.send_file(sock, './model_para/xy_files2_test.tensor', far_ip)
                u.recv_string(sock)
    return pred
