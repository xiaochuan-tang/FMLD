...
def train(model,
    ...
    for i in range(client_number):
        ack, addr = u.recv_string(sock)
        farClient.append(addr)
    ...
    while iter < iters:
        for data in loader:
            ...
            if fp16:
                ...
                for i in range(client_number):
                    u.send_string(sock, "begin", farClient[i])
                    # model_para/server
                    u.recv_file(sock, './model_para/rev_xy_files' + str(i + 1) + '.tensor')
                    u.send_string(sock, "finish", farClient[i])

                for i in range(client_number):
                    model_params = paddle.load('./model_para/rev_xy_files' + str(i + 1) + '.tensor')
                    para_list.append(model_params)
                ...
                if opt_grad is None:
                    print("grad is none")
                    print(opt_grad)
                paddle.save(opt_grad, "./model_para/server_dfx_grad1.tensor")
                paddle.save(dem_grad, "./model_para/server_dfx_grad2.tensor")
                paddle.save(sar_grad, "./model_para/server_dfx_grad3.tensor")
                # send_file(server1, "./model_para/server/server_dfx_grad1.tensor")
                # send_file(server2, "./model_para/server/server_dfx_grad2.tensor")
                # send_file(server3, "./model_para/server/server_dfx_grad3.tensor")
                for i in range(client_number):
                    u.send_file(sock, './model_para/server_dfx_grad' + str(i + 1) + '.tensor', farClient[i])
                    u.recv_string(sock)
            ...
            if (iter % save_interval == 0
                ...
                mean_iou, acc, F1, _, _, _ = evaluate(
                    model, val_dataset, sock, client_number, farClient, num_workers=num_workers, **test_config)
