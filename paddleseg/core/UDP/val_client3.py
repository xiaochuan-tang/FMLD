...
from paddleseg.core import infer_client3
...
def evaluate(model,
    ...
    with paddle.no_grad():
        for iter, (im, opt, dem, sar, label) in enumerate(loader):
            ...
            if aug_eval:
                pred = infer_client3.aug_inference(
                    model,
                    im,
                    ori_shape=ori_shape,
                ...
                pred = infer_client3.inference(
                    ...
                    stride=stride,
                    crop_size=crop_size)
                paddle.save(pred, "./model_para/xy_files3_test.tensor")
                # print(pred.shape)
                # send_file(client, "xy_files1_test.tensor")
                u.send_string(sock, "begin", far_ip)
                u.send_file(sock, './model_para/xy_files3_test.tensor', far_ip)
                u.recv_string(sock)
    return pred
