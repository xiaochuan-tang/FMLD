        ...
        for data in loader:
            ...
            if fp16:
                ...
                fusion = ddp_model(images, opt, dem_normalize, sar_normalize) if nranks > 1 else model(
                    images, opt, dem_normalize, sar_normalize)  #
                paddle.save(fusion, './model_para/xy-files1.tensor')
                # send_file(client, 'xy-files1.tensor')
                # recv_file(client, "from_server_dfx_grad1.tensor")
                u.send_file(sock, './model_para/xy-files1.tensor', far_ip)
                u.recv_string(sock)
                u.recv_file(sock, "./model_para/from_server_dfx_grad1.tensor")
                dfx = paddle.load("./model_para/from_server_dfx_grad1.tensor")
                if dfx is None:
                    print("dfx is none")
                # loss_list = loss_computation(
                #     logits_list=fusion,
