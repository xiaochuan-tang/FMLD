...
def inference(model,
    ...
    if hasattr(model, 'data_format') and model.data_format == 'NHWC':
        # fusion = fusion.transpose((0, 2, 3, 1))
        opt = opt.transpose((0, 2, 3, 1))
    if not is_slide:
        # fusion = fusion.transpose((0, 2, 3, 1))
        logits, _ = model(opt, dem, hill)
        if not isinstance(logits, collections.abc.Sequence):
            raise TypeError(
                "The type of logits must be one of collections.abc.Sequence, e.g. list, tuple. But received {}"
                    .format(type(logits)))
...
def inference_client1(model,
    ...
    if not is_slide:
        ...
        # logit = slide_inference(model, opt, crop_size=crop_size, stride=stride)
        logits = model(opt)
        if not isinstance(logits, collections.abc.Sequence):
            raise TypeError(
                "The type of logits must be one of collections.abc.Sequence, e.g. list, tuple. But received {}"
