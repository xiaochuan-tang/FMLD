...
from paddleseg.core.val_client3 import evaluate
    ...
    while iter < iters:
        for data in loader:
            ...
            if fp16:
                ...
                logits_list = ddp_model(sar_normalize) if nranks > 1 else model(sar_normalize)  #
                # print(logits_list.shape)
                paddle.save(logits_list, 'xy-files3.tensor')
                send_file(client, 'xy-files3.tensor')

                recv_file(client, "from_server_dfx_grad3.tensor")
                dfx = paddle.load("from_server_dfx_grad3.tensor")
                ...
                #     images, opt, dem_normalize, sar_normalize)  #
                # paddle.save(forward2, 'xy-files3-fusion.tensor')
                # send_file(client, 'xy-files3-fusion.tensor')
    ...
    while iter < iters:
        for data in loader:
            ...
            if fp16:
                ...
                # dem_normalize.stop_gradient = True
                # sar_normalize.stop_gradient = True
                forward2 = ddp_model(images) if nranks > 1 else model(images)  #
                paddle.save(forward2, 'xy-files3-fusion.tensor')
                send_file(client, 'xy-files3-fusion.tensor')
                recv_cmd(client)
