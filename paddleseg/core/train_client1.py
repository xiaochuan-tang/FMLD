    ...
    while iter < iters:
        for data in loader:
            ...
            if fp16:
                ...
                logits_list = ddp_model(opt) if nranks > 1 else model(opt)  # 客户端预测的pred
                # print(logits_list.shape)
                paddle.save(logits_list, 'xy-files1.tensor')
                send_file(client, 'xy-files1.tensor')  # 将pred发送给服务器

                recv_file(client, "from_server_dfx_grad1.tensor")  # 接受来自服务器的dfx
                ...
                #     images, opt, dem_normalize, sar_normalize)  #
                # paddle.save(forward2, 'xy-files1-fusion.tensor')
                # send_file(client, 'xy-files1-fusion.tensor')
    ...
    while iter < iters:
        for data in loader:
            ...
            if fp16:
                ...
                # dem_normalize.stop_gradient = True
                # sar_normalize.stop_gradient = True
                forward2 = ddp_model(opt) if nranks > 1 else model(opt)  #
                paddle.save(forward2, 'xy-files1-fusion.tensor')
                send_file(client, 'xy-files1-fusion.tensor')
                recv_cmd(client)
