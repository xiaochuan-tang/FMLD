    ...
    def __init__(self,
        ...
        with open(file_path, 'r') as f:
            for line in f:
                ...
                image = os.path.join(self.dataset_root, items[0])
                opt = os.path.join(self.dataset_root, items[1])
                dem = os.path.join(self.dataset_root, items[2])
                sar = os.path.join(self.dataset_root, items[3])
                label_path = os.path.join(self.dataset_root, items[4])

                self.file_list.append([image, opt, dem, sar, label_path])

    def __getitem__(self, idx):
        # image_path, label_path = self.file_list[idx]
        image_path, opt, dem, sar, label_path = self.file_list[idx]
        # 山体阴影，光学影像，dem，sar，lable
        # print("self.file_list[idx].sar:", sar)    dataset/dataset_MMLD/train_sar_tif/810.tif
        # print(dem_path)
        if self.mode == 'test':
            im, _ = self.transforms(im=image_path)
            opt, _ = self.transforms(im=opt)
            # remoteimg,_ = self.transforms(im=remote_path)
            ...
            sar = cv2.resize(sar, (1024, 1024))
            sar = sar[np.newaxis, :, :]
            # print("sar.shape:", sar.shape)    (1, 1024, 1024)

            if self.edge:
                edge_mask = F.mask_to_binary_edge(
                    label, radius=2, num_classes=self.num_classes)
                return im, opt, dem, sar, label, edge_mask
            else:
                return im, opt, dem, sar, label
